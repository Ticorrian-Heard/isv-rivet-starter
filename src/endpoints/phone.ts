import { phoneS2SOAuthClient, logger, app, db } from '../modules.ts';
import express from 'express';
import { parse } from 'url';

export const startPhoneEndpoints = () => {
    // app.get('/call_logs', async (req: express.Request, res: express.Response)=>{
    ...
    //         return;
    //       } 
    //       logger.info(['meeting retrieved', user?.phone]);
    //       res.status(200).send({success: 'meeting retrieved', response: user?.phone});
    //     } catch (err) {
    //         logger.error([err]);
    ...
    // app.get('/voicemail_logs', async (req: express.Request, res: express.Response)=>{
    //     let q = parse(req.url, true).query;
    ...
    //     try {
    //       let responseData: object = await phoneS2SOAuthClient.endpoints.phone.createMeeting({ body, path });
    ...
    //           if (user) user.phone?.push(meetingObj) 
    //             else {
    //               logger.info(['user' + q.email + ' not found in db'])
    //               res.status(404).send({error: q.email + ' not found in db'});
    //               return;
    ...
    // app.get('/callrecording_logs', async (req: express.Request, res: express.Response)=>{
    //     let q = parse(req.url, true).query;
    ...
    //     let meetingId = user?.phone?.find((mtg) => mtg.meetingId === parseInt(<string>q.meetingId))?.meetingId;
    ...
    //       db.update(({ users }) => {
    //         let user = users.find((obj) => obj.email === <string>q.email); 
    //         if (user) user.phone = user.phone?.filter((mtg) => mtg.meetingId !== path.meetingId);
    //       });

    //       let responseData: object = await phoneS2SOAuthClient.endpoints.phone.deleteMeeting({ path });
    ...
    // app.get('/sms_logs', async (req: express.Request, res: express.Response)=>{
    //   let q = parse(req.url, true).query;
    ...
    // app.get('/user_logs', async (req: express.Request, res: express.Response)=>{
    //   let q = parse(req.url, true).query;
